...
Внедрение с помощью интерфейса похоже на предыдущий подход, только в нём используются не сеттеры, а отдельные методы-инжекторы. Их мы описываем в интерфейсе `BuildingDependencies`, который реализует класс `Building`. Внутри функции `Builder` мы вызываем инжекторы, передавая как аргумент нужную зависимость.
