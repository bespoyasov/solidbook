...
В идеально-спроектированной системе модули зависят только от абстракций, сцепление — минимальна, а связность — максимальна.
...
Если мы сменим базу данных, то нам придётся менять и код модуля `Auth`. По-хорошему `Auth` не должен ничего знать о базе данных, которую мы используем. Ему достаточно знать, что есть какая-то база, к которой можно достучать через определённые методы — это работа интерфейса.
...
Исправленный вариант автоматически удовлетворяет двум другим принципам: OCP и LSP.
