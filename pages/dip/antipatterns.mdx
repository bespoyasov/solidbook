...
Основная проблема антипаттернов и запахов ниже — в сокрытии зависимостей, что увеличивает сцепление и делает тестирование трудным.
...
Теперь `Auth` жёстко сцеплен с `MySQlConnection`, то есть _не сможет работать без него_. Также чтобы протестировать такой класс, придётся сделать _глобальный_ мок для `MySQlConnection`.
...
При испольльзовании DI такой проблемы нет — у нас просто не получится создать экземпляр класса,не передав все необходимые зависимости.
