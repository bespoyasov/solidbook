...
interface ApplicantObserver extends Applicant {
  update(position: string): void
}

class SoftwareEngineerApplicant implements ApplicantObserver {
  name: string
  position: string
...
interface PositionObservable extends Observable {
  append(position: string): void
  listeners: ApplicantObserver[]
}

class HrAgency implements PositionObservable {
  listeners: ApplicantObserver[] = []

  subscribe = (applicant: ApplicantObserver): void => {
    this.listeners.push(applicant)
  }

  unsubscribe = (applicant: ApplicantObserver): void => {
    this.listeners = this.listeners.filter(listener => listener.name !== applicant.name)
  }
