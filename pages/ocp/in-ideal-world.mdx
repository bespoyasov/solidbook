...
Это по сути своей ограничение на поведение сущностей внутри системы — гипотеза того, как они друг с другом взаимодействуют. В целом люди плохо умеют прогнозировать и предсказывать. И хотя опытный проектировщик, имея достаточно знаний о проектируемой системе, может сделать хорошее предположение, OCP предполагает методику [Just-in-time design](https://agiledesign.org/2015/01/18/jit-just-in-time-and-software-development/). Она предполагает внесение изменений и добавление сущностей по мере необходимости, не раньше.

Вернёмся к примеру. Сейчас классы фигур будут подчиняться новому ограничению и имплементировать интерфейс `Shape`:
