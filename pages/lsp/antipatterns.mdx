...
Для подобной задачи подходит шаблон [Cостояние](https://refactoring.guru/ru/design-patterns/state) — он позволяет менять поведение объектов в зависимости от их внутреннего состояния. Если он реализован правильно и полно, то LSP он не нарушит. Однако реализация в примере ниже нарушает.
