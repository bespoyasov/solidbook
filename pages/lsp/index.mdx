...
```ts
function testShapeSize(r: Rectangle) {
  r.setWidth(10)
  r.setHeight(20)
  assert(r.width * r.height === 200)
  // условие не сработает, если r — экземпляр класса Square
}
```
...
Сам принцип звучит так: функции, которые используют базовый тип, должны иметь возможность использовать подтипы базового типа, не зная об этом.

Если простыми словами, то классы-наследники не должны противоречить базовому классу. Тогда поведение таких классов будет ожидаемым для функций, которые используют базовый класс.
...
```ts
const oldHeight = r.height
r.setWidth(newWidth)

assert((r.width === newWidth) && (r.height === oldHeight))
```
