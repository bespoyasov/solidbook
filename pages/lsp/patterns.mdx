...
В прошлых разделах, когда мы выделяли общий интерфейс для классов, которые не вписывались в существующую иерархию.

Для подобной работы подходят приёмы [извлечения интерфейса](https://refactoring.guru/ru/extract-interface) и [ивзлечения суперкласса](https://refactoring.guru/ru/extract-superclass). В [примере с иерархией пользователей](/lsp/in-real-life) мы использовали выделение суперкласса.
...
Среди создающих и сруктурных шаблонов можно условно выделить группу тех, которые помогают использовать композицию свойств и методов. Примерами могут послужить стратегия и декоратор, которые мы рассмотрели в [разделе OCP](/ocp/patterns).
...
- [Ивзлечение суперкласса](https://refactoring.guru/ru/extract-superclass)
