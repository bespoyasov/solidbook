...
LSP помогает выявлять проблемные абстракции при проектировании и строить ирерархию сущностей с учётом подобных проблем.
...
Ещё один пример LSP — это React-компоненты, а точнее их реализация в JSX. [Синтаксис JSX](https://reactjs.org/docs/introducing-jsx.html) построен таким образом, что мы можем писать разметку компонентов в HTML-подобном виде:
...
Возможность использовать компоненты точно так же, как «обычные теги», — реализация принципа подстановки Лисков. Мы можем заменить «обычный тег» на компонент, потому что и те и другие — это реализация `ReactElement`, который описывает, как именно они должны себя вести.
