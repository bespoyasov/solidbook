...
Первый способ — переделать иерархию так, чтобы `Square` не наследовался от `Rectangle`. Мы можем ввести новый класс, чтобы и квадрат, и прямоугльник наследовались от него.
...
Теперь с помощью интерфейсов мы можем композировать свойства, которые необходимо реализовать для сущностей. Мы автоматически соблюдаем [OCP](/ocp), избегая прямого наследования и привязывась к абстракциям, а не к конкретным классам.
