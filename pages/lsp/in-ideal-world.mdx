...
Теперь с помощью интерфейсов мы можем композировать свойства, которые необходимо реализовать для сущностей. Мы автоматически соблюдаем [OCP](/ocp), избегая прямого наследования и привязываясь к абстракциям, а не к конкретным классам.
...
_Содержит и описывает в себе общую для обоих классов функциональность_

_Предотвращает противоречие в поведении базовой сущности и её потомков_

_Позволяет подменять `Square` и `Rectangle` друг другом во всех функциях, которые используют их общий интерфейс_

_Позволяет композировать свойства без прямого наследования_

**Допустим, требуется добавить круг и эллипс. Какой из вариантов добавления не будет противоречить LSP?**

Вариант 1 (Прямое наследование здесь не подойдёт, потому что поведение эллипса отличается от поведения круга. Аналогично квадрату и прямоугольнику необходимо ввести общий базовый тип для обеих фигур.)
```ts
class Ellips {/*...*/}
