...
## Вопросы
**В чём суть проблемы грязного интерфейса?**

_Он содержит в себе слишком много методов и полей, не все из которых могут быть нужны сущностям, реализующим его_

_Он содержит в себе слишком много методов и полей, что может привести к плохой читаемости и дорогой поддержки_

Он не может быть разбит на несколько более мелких, из-за чего его трудно и дорого поддерживать (Один из способо борьбы с грязным интерфейсом — разбиение его на более мелкие.)

**Какие приёмы помогают избегать грязных интерфейсов или рефакторить их?**

Расширение интерфейсов от более широкого родительского (Наоборот, если родительский интерфейс имеет больше методов и полей, то выше вероятность, что один из них будет не нужен в сущности, реализующей этот интерфейс)

_Разбиение на интерфейсы, содержащие меньшее количество методов_

_Выделение интерфейса, извлечение класса, суперкласса_

**В чём заключается опасность «пустой» реализации интерфейса?**

_«Пустая» реализация исполняет требования интерфейса лишь формально, что создаёт ложные ожидания от описанного метода_

«Пустая» реализация — это необходимая издержка, с помощью которой можно построить структуру, удовлетворяющую ISP (ISP напротив требует проектировать интерфейсы так, чтобы необходимости в «пустых» реализациях не было.)

_«Пустая» реализация создаёт не даёт гарантий, что метод будет удовлетворять контракту или ожиданиям от него_
