...
Для решения этой задачи воспользуемся шаблоном проектирования [«Стратегия»](https://refactoring.guru/ru/design-patterns/strategy), который поможет выбрать подходящий формат. (Более подробно «Стратегию» мы разберём в [разделе о принципе открытости и закрытости](/ocp/patterns).)
