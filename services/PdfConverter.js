...
var require_is_buffer = __commonJS({
  "node_modules/unified/node_modules/is-buffer/index.js"(exports, module2) {
  ...
  "node_modules/micromark-extension-mdxjs/node_modules/acorn/dist/acorn.js"(exports, module2) {
    (function(global, factory) {
      ...
      function pushComment2(options2, array) {
        return function(block2, text4, start3, end, startLoc, endLoc) {
          var comment = {
      ...
      Parser4.prototype.parse = function parse5() {
      ...
      Parser4.parse = function parse5(input, options2) {
...
var VFile = class {
  constructor(value) {
    ...
    while (++index2 < order.length) {
      ...
      if (prop2 in options2 && options2[prop2] !== void 0) {
...
function toString(node, options2) {
...
function parse(options2 = {}) {
  const constructs2 = combineExtensions([constructs_exports].concat(options2.extensions || []));
  const parser2 = {
    defined: [],
    lazy: {},
...
var fromMarkdown = function(value, encoding, options2) {
...
function remarkParse(options2) {
  const parser2 = (doc) => {
    ...
    return fromMarkdown(doc, Object.assign({}, settings, options2, {
...
function eventsToAcorn(events, options2) {
  const { prefix = "", suffix = "" } = options2;
  const comments = [];
  const acornConfig = Object.assign({}, options2.acornOptions, {
    onComment: comments,
    preserveParens: true
  });
  const chunks = [];
  ...
  let startLine;
  if (options2.start) {
    startLine = options2.start.line;
    lines[startLine] = options2.start;
  }
  while (++index2 < events.length) {
...
function mdxExpression(options2 = {}) {
  const addResult = options2.addResult;
  const acorn = options2.acorn;
  const spread = options2.spread;
  let allowEmpty = options2.allowEmpty;
  let acornOptions;
  ...
  } else if (options2.acornOptions || options2.addResult) {
...
function mdxJsx(options2 = {}) {
  ...
  return {
    flow: {
      [60]: jsxFlow(acorn, acornOptions, options2.addResult)
    },
    text: {
      [60]: jsxText(acorn, acornOptions, options2.addResult)
    }
  };
}
...
function parseEntities(value, options2 = {}) {
  ...
  if (options2.position) {
    if ("start" in options2.position || "indent" in options2.position) {
      indent = options2.position.indent;
      point2 = options2.position.start;
    } else {
      point2 = options2.position;
    }
  }
    ...
    if (character === 38) {
      ...
      if (reference) {
        ...
        if (options2.reference) {
          options2.reference.call(options2.referenceContext, reference, { start: previous2, end: next }, value.slice(start3 - 1, end));
        }
        previous2 = next;
      } else {
  ...
  function warning(code2, offset2) {
    ...
    if (options2.warning) {
      ...
      options2.warning.call(options2.warningContext, messages[code2], position2, code2);
    }
  }
  function flush() {
    if (queue) {
...
function mdxJsxFromMarkdown() {
  ...
  function onErrorRightIsTag(closing, open) {
    ...
    throw new VFileMessage("Expected a closing tag for `" + serializeAbbreviatedTag(tag) + "` (" + stringifyPosition({ start: open.start, end: open.end }) + ")" + place, position2, "mdast-util-mdx-jsx:end-tag-mismatch");
  }
...
// node_modules/mdast-util-to-markdown/node_modules/zwitch/index.js
...
function code(node, _, context, safeOptions) {
  ...
  if (node.lang) {
    ...
    value += tracker.move(safe(context, node.lang, {
      before: value,
      after: " ",
      encode: ["`"],
      ...tracker.current()
...
function definition2(node, _, context, safeOptions) {
  ...
  if (!node.url || /[\0- \u007F]/.test(node.url)) {
    ...
    value += tracker.move("<");
    value += tracker.move(safe(context, node.url, { before: value, after: ">", ...tracker.current() }));
    value += tracker.move(">");
  } else {
...
function image(node, _, context, safeOptions) {
  ...
  const tracker = track(safeOptions);
  let value = tracker.move("![");
  value += tracker.move(safe(context, node.alt, { before: value, after: "]", ...tracker.current() }));
  value += tracker.move("](");
  subexit();
...
function link(node, _, context, safeOptions) {
  ...
  if (formatLinkAsAutolink(node, context)) {
    ...
    value2 += tracker.move(containerPhrasing(node, context, {
      before: value2,
      after: ">",
      ...tracker.current()
    }));
...
function listItem(node, parent, context, safeOptions) {
  ...
  const value = indentLines(containerFlow(node, context, tracker.current()), map3);
  exit2();
  return value;
...
function remarkStringify(options2) {
  const compiler2 = (tree) => {
    const settings = this.data("settings");
    return toMarkdown(tree, Object.assign({}, settings, options2, {
...
// services/PdfConverter.ts
var import_md_to_pdf = require("md-to-pdf");
...
function indentCodeCompensation(raw, text4) {
...
var Tokenizer = class {
  ...
  code(src) {
    ...
    if (cap) {
      const text4 = cap[0].replace(/^ {1,4}/gm, "");
      return {
        type: "code",
        raw: cap[0],
  ...
  fences(src) {
    ...
    if (cap) {
      const raw = cap[0];
      const text4 = indentCodeCompensation(raw, cap[3] || "");
      return {
        type: "code",
        raw,
  ...
  blockquote(src) {
    ...
    if (cap) {
      const text4 = cap[0].replace(/^ *>[ \t]?/gm, "");
      return {
        type: "blockquote",
        raw: cap[0],
        tokens: this.lexer.blockTokens(text4, []),
        text: text4
      };
    }
  }
  list(src) {
    ...
    if (cap) {
      let raw, istask, ischecked, indent, i, blankLine2, endsWithBlankLine, line, nextLine, rawLine, itemContents, endEarly;
      let bull = cap[1].trim();
    ...
    if (!nextChar || nextChar && (prevChar === "" || this.rules.inline.punctuation.exec(prevChar))) {
      ...
      while ((match = endReg.exec(maskedSrc)) != null) {
        ...
        if (Math.min(lLength, rLength) % 2) {
          const text5 = src.slice(1, lLength + match.index + rLength);
          return {
            type: "em",
        ...
        const text4 = src.slice(2, lLength + match.index + rLength - 1);
        return {
          type: "strong",
          raw: src.slice(0, lLength + match.index + rLength + 1),
          text: text4,
  ...
  codespan(src) {
    ...
    if (cap) {
      let text4 = cap[2].replace(/\n/g, " ");
      const hasNonSpaceChars = /[^ ]/.test(text4);
      const hasSpaceCharsOnBothEnds = /^ /.test(text4) && / $/.test(text4);
      if (hasNonSpaceChars && hasSpaceCharsOnBothEnds) {
        text4 = text4.substring(1, text4.length - 1);
      }
      text4 = escape(text4, true);
      return {
        type: "codespan",
        raw: cap[0],
        text: text4
  ...
  autolink(src, mangle2) {
    ...
    if (cap) {
      let text4, href;
      if (cap[2] === "@") {
        text4 = escape(this.options.mangle ? mangle2(cap[1]) : cap[1]);
        href = "mailto:" + text4;
      } else {
        text4 = escape(cap[1]);
        href = text4;
  ...
  url(src, mangle2) {
    ...
    if (cap = this.rules.inline.url.exec(src)) {
      let text4, href;
      if (cap[2] === "@") {
        text4 = escape(this.options.mangle ? mangle2(cap[0]) : cap[0]);
        href = "mailto:" + text4;
      } else {
        let prevCapZero;
        do {
  ...
  inlineText(src, smartypants2) {
    ...
    if (cap) {
      ...
      if (this.lexer.state.inRawBlock) {
        text4 = this.options.sanitize ? this.options.sanitizer ? this.options.sanitizer(cap[0]) : escape(cap[0]) : cap[0];
      } else {
        text4 = escape(this.options.smartypants ? smartypants2(cap[0]) : cap[0]);
      }
      return {
        type: "text",
        raw: cap[0],
...
function smartypants(text4) {
  return text4.replace(/---/g, "\u2014").replace(/--/g, "\u2013").replace(/(^|[-\u2014/(\[{"\s])'/g, "$1\u2018").replace(/'/g, "\u2019").replace(/(^|[-\u2014/(\[{\u2018\s])"/g, "$1\u201C").replace(/"/g, "\u201D").replace(/\.{3}/g, "\u2026");
}
function mangle(text4) {
  let out = "", i, ch;
  const l = text4.length;
  for (i = 0; i < l; i++) {
    ch = text4.charCodeAt(i);
    if (Math.random() > 0.5) {
      ch = "x" + ch.toString(16);
    }
    out += "&#" + ch + ";";
...
`;
  ...
  heading(text4, level, raw, slugger) {
    if (this.options.headerIds) {
      const id = this.options.headerPrefix + slugger.slug(raw);
      return `<h${level} id="${id}">${text4}</h${level}>
`;
    }
    return `<h${level}>${text4}</h${level}>
`;
  }
  hr() {
...
`;
  ...
  strong(text4) {
    return `<strong>${text4}</strong>`;
  }
  em(text4) {
    return `<em>${text4}</em>`;
  }
  codespan(text4) {
    return `<code>${text4}</code>`;
  }
  br() {
    return this.options.xhtml ? "<br/>" : "<br>";
  }
  del(text4) {
    return `<del>${text4}</del>`;
  }
  link(href, title, text4) {
...
var Parser3 = class {
  ...
  static parse(tokens, options2) {
    const parser2 = new Parser3(options2);
  ...
  parse(tokens, top = true) {
    let out = "", i, j, k, l2, l3, row, cell, header, body, token, ordered, start3, loose, itemBody, item, checked, task, checkbox, ret;
...
function marked(src, opt, callback) {
  ...
  try {
    ...
    return Parser3.parse(tokens, opt);
  } catch (e) {
...
marked.use = function(...args) {
  ...
  args.forEach((pack) => {
    ...
    if (pack.tokenizer) {
      const tokenizer2 = marked.defaults.tokenizer || new Tokenizer();
      for (const prop in pack.tokenizer) {
        const prevTokenizer = tokenizer2[prop];
        tokenizer2[prop] = (...args2) => {
          let ret = pack.tokenizer[prop].apply(tokenizer2, args2);
          if (ret === false) {
...
marked.Parser = Parser3;
marked.parser = Parser3.parse;
...
var parser = Parser3.parse;
var lexer = Lexer.lex;

// services/PdfConverter.ts
var { promises: fs } = import_fs.default;
var parseConfig = unified().use(remark_parse_default).use(remarkMdx);
var stringifyConfig = unified().use(remark_stringify_default).use(remarkMdx);
var invalidTypes = ["mdxjsEsm", "mdxJsxFlowElement"];
var glossary = {};
var pages = [
  ["pages/index.mdx"],
  [
...
var regexForTitle = new RegExp("title: '(.*)'", "");
var regexForSuggest = new RegExp("\\*\\[(.*)\\]:(.*)", "gm");
var generateTableOfContent = (md) => {
  const toc = [];
  const renderer = new marked.Renderer();
  renderer.heading = (text4, level, raw, slugger) => {
    const slug = slugger.slug(raw);
    toc.push({ level, text: text4, slug });
    return text4;
  };
  marked(md, { renderer });
  const tableOfContent = "# \u0421\u043E\u0434\u0435\u0440\u0436\u0430\u043D\u0438\u0435\n" + toc.map((t) => `${Array(t.level).join("  ")}- [${t.text}](#${t.slug})`).join("\n\n") + `
...
var formatMarkdown = async (parts) => {
  let newMd = "";
  parts.forEach((files) => {
    files.forEach((file) => {
      const data2 = import_fs.default.readFileSync(file, "utf8");
      const treeRoot = parseConfig.parse(data2);
      visit2(treeRoot, "link", (node) => {
        if (node.url[0] === "/") {
          node.url = node.url.replace("/", "#");
        }
      });
      const changeNode = (el, index2) => {
...
`;
        ...
        if (invalidTypes.includes(el.type)) {
          ...
          return false;
        } else if (index2 === treeRoot.children.length - 1 && el.type === "paragraph") {
          let m = null;
...
var generateGlossary = () => {
  let gloss = "";
  const keys2 = Object.keys(glossary).sort();
  keys2.forEach((definite) => {
...
var generatePDF = async (text4) => {
  const pdf = await (0, import_md_to_pdf.mdToPdf)({ content: text4 }, { document_title: "SOLID book" }).catch(console.error);
  if (pdf) {
